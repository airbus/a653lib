...
etc.) are exposed to the Wasm module via the runtime's import mechanism.
...
[`c-abi-lens`](https://github.com/DLR-FT/arinc653-wasm) directly from the
[ARINC 653 headers](https://www.aviation-ia.com/support-files/arinc653h);
...
Tested on **ArchLinux**, **Ubuntu** (see CI), and **RHEL7** (native mode only).
Expected to work on any Linux 64-bit host.
...
Install **WAMR** (libiwasm) from source (WAMR-2.4.4):
...
Install **Wasmtime C API** (v45.0.0):
...
The sysroot will be at `/opt/wasi-sdk/share/wasi-sysroot`.
...
The following files will be present in the `build/` directory:
...
- Not all APEX functions are implemented; the demonstration covers partitions with
  processes using sampling and queuing ports. Unimplemented functions are stubbed
  as empty and will be added incrementally as needed.
