...
static int sizeFifo (unsigned int max_elem, 
                     unsigned int max_elem_size){
  
  int space_needed = 0;

  /*   int headerSize =  */
  /*     sizeof(t_queuing_port_shm_data) +  */
  /*     max_elem * sizeof(fifo_msg_header_t*) + */
  /*     max_elem * sizeof(fifo_msg_header_t) + */
  /*     max_elem * max_size; */

  space_needed  =  sizeof(t_queuing_port_shm_data);  /* !!!! will be 20 and not 24 !!!!! */
  space_needed +=  max_elem * sizeof(fifo_msg_header_t);
  space_needed +=  max_elem * max_elem_size;
...
void create_queuing_port_ip (QUEUING_PORT_NAME_TYPE  QUEUING_PORT_NAME,
  ...
  *RETURN_CODE = NO_ACTION;
  ...
  RETURN_CODE_TYPE MY_RETURN_CODE;
  int idx   = 0;
  int found = 0;
